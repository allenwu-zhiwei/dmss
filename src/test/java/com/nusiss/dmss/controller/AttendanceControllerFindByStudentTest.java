...
class AttendanceControllerFindByStudentTest {

    @Autowired
    private MockMvc mockMvc; // 模拟 MVC 的 HTTP 请求与响应

    @MockBean(name = "findAttendanceByStudentStrategy")
    private AttendanceOperation findAttendanceByStudentStrategy; // Mock 考勤创建策略
    ...
    @Test
    void testFindAttendanceByStudent() throws Exception {
        AttendanceRecord record = new AttendanceRecord();
        record.setStudentId(1);
        record.setCourseId(101);
        record.setStatus("Present");

        when(findAttendanceByStudentStrategy.findAttendanceByStudentId(1)).thenReturn(Collections.singletonList(record));

        mockMvc.perform(get("/api/v1/attendance/student/{studentId}", 1))
