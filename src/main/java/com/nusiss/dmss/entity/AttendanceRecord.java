...
public class AttendanceRecord {
    ...
    //学生ID不为空
    @Column(name = "student_id", nullable = false)   //该列在数据库中不能为 NULL,需为有效值
    ...
    private String status;  //出勤状态，如 "Present", "Absent", "Late"，不能为空

    @Column(name = "remarks")
    private String remarks;  // 备注信息（可选）,可选则可以为空

    @Column(name = "create_datetime", nullable = false)
    private LocalDateTime createDatetime;   // 创建出勤记录的时间，不能为空
